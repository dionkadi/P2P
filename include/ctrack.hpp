...
{
public:
    AsyncEventHandler(int line = __builtin_LINE(), 
    ...
    ~AsyncEventHandler()
    {
        ...
        // Fetch the thread context for the CURRENT thread (the one we woke up on)
        // This guarantees event_ptr is initialized and safe to use.
        int current_t_id = fetch_event_t_id();

        // Generate a new event ID, but DO NOT touch the parent/child hierarchy 
        // (we ignore previous_event_id and current_event_id assignments)
        unsigned int async_event_id = ++(*current_event_cnt);

        // Safely push to the resuming thread's queue
        if (event_ptr->capacity() - event_ptr->size() < 1)
            event_ptr->reserve(event_ptr->capacity() * 4);

        event_ptr->emplace_back(Event{
            start_time, end_time, filename, line, function, 
            current_t_id, async_event_id
        });
    }
...
#define CTRACK_ASYNC_IMPL_NAME(name) \
    ctrack::AsyncEventHandler CTRACK_UNIQUE_NAME(ctrack_async_instance_) { __builtin_LINE(), __builtin_FILE(), name }

#define CTRACK_ASYNC(name) CTRACK_ASYNC_IMPL_NAME(name)
