...
public:
    AsyncRateLimiter(asio::io_context& io_context, uint64_t rate_bps, uint64_t capacity_factor = 2)
        ...
        if (rate_bytes_per_second_ > 0) {
            // Post the initial call to ensure it runs on the strand
            asio::post(strand_, [this] { refill_tokens(); });
        }
    }

    // This function now returns a standard asio::awaitable, which composes perfectly.
    asio::awaitable<void> await_tokens(size_t amount) {
        ...
        // Use async_initiate to create a new awaitable asynchronous operation.
        // The signature is the type of the completion handler.
        co_await asio::async_initiate<void(boost::system::error_code)>(
            [this, amount](auto&& completion_handler) {
                // This lambda is the initiation function. It's called immediately.
                // We dispatch the logic onto our strand to ensure thread safety.
                asio::dispatch(strand_, [this, amount, h = std::move(completion_handler)]() mutable {
                    if (tokens_ >= amount) {
                        ...
                        asio::post(strand_, [handler = std::move(h)]() mutable {
                            handler(boost::system::error_code{}); // Pass empty error_code for success
                        });
                    } else {
...
private:
    void refill_tokens() {
        // This function is always called from the strand.
    ...
    // The queue now stores {amount_needed, completion_handler}
